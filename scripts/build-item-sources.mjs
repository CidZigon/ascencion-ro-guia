...
function parseNpcLine(line, shops, spawns) {
...
function addDrop(byItem, itemId, monster, drop, maps) {
  const entry = byItem.get(itemId) ?? { drops: [], shops: [] };
...
function addShop(byItem, shop) {
  const entry = byItem.get(shop.itemId) ?? { drops: [], shops: [] };
...
try {
  ...
  for (const file of await filesInside(join(root, "npc"))) {
    ...
    for (const line of content.split(/\r?\n/)) parseNpcLine(line, shops, spawns);
  }
  ...
  console.log(`Fuentes generadas: ${sourcedItems} objetos, ${dropLinks} drops, ${shops.length} filas de tienda.`);
} finally {
